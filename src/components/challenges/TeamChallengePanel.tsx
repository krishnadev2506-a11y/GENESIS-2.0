...
export function TeamChallengePanel({ initialFeature, initialSituation, phase1Open, phase2Open, featureWheelEntries, situationWheelEntries }: { initialFeature?: Assignment; initialSituation?: Assignment; phase1Open: boolean; phase2Open: boolean; featureWheelEntries?: string[]; situationWheelEntries?: string[] }) {
  ...
  const pendingFeature = useRef<Assignment | null>(null);
  ...
  const prepareFeature = async () => {
    try {
      beginSpin('feature');
      const assignment = await request('feature');
      pendingFeature.current = assignment;
  ...
  const finishFeature = async () => {
    if (pendingFeature.current) setFeature(pendingFeature.current);
  ...
  const renderPhase = (phase: Phase, assignment?: Assignment) => {
    ...
    if (!assignment) return <section className={`rounded-2xl border ${accent} p-6`}><p className="text-xs font-mono uppercase tracking-[.14em] text-text-muted">{label}</p><h2 className="mt-2 text-xl font-display font-bold text-white">{selectionOpen ? 'Ready to spin' : 'Selection is closed'}</h2><p className="mt-2 text-sm text-text-muted">{selectionOpen ? 'Your team can generate this assignment once. The result is saved permanently.' : 'Challenge selection will be opened by the event administrators.'}</p><ChallengeSpinWheel phase={phase} entries={wheelEntries} disabled={loading !== null || !selectionOpen} onSpinStart={() => {}} prepareSpin={isFeature ? prepareFeature : prepareSituation} onSpinComplete={isFeature ? finishFeature : finishSituation} /></section>;
