...
export function TeamChallengePanel({ initialFeature, initialSituation, phase1Open, phase2Open }: { initialFeature?: Assignment; initialSituation?: Assignment; phase1Open: boolean; phase2Open: boolean }) {
  const [feature, setFeature] = useState(initialFeature);
  const [situation, setSituation] = useState(initialSituation);
  const [loading, setLoading] = useState<Phase | null>(null);
  const [error, setError] = useState('');

  const beginSpin = (phase: Phase) => {
    setLoading(phase);
    setError('');
  };
  ...
  const renderPhase = (phase: Phase, assignment?: Assignment) => {
    ...
    if (!assignment) return <section className={`rounded-2xl border ${accent} p-6`}><p className="text-xs font-mono uppercase tracking-[.14em] text-text-muted">{label}</p><h2 className="mt-2 text-xl font-display font-bold text-white">{selectionOpen ? 'Ready to spin' : 'Selection is closed'}</h2><p className="mt-2 text-sm text-text-muted">{selectionOpen ? 'Your team can generate this assignment once. The result is saved permanently.' : 'Challenge selection will be opened by the event administrators.'}</p><ChallengeSpinWheel phase={phase} disabled={loading !== null || !selectionOpen} onSpinStart={() => beginSpin(phase)} onSpinComplete={() => spin(phase)} /></section>;
