...
export function TeamChallengePanel({ initialFeature, initialSituation, phase1Open, phase2Open, situationWheelEntries }: { initialFeature?: Assignment; initialSituation?: Assignment; phase1Open: boolean; phase2Open: boolean; situationWheelEntries: string[] }) {
  ...
  const renderPhase = (phase: Phase, assignment?: Assignment) => {
    ...
    if (!assignment) return <section className={`rounded-2xl border ${accent} p-6`}><p className="text-xs font-mono uppercase tracking-[.14em] text-text-muted">{label}</p><h2 className="mt-2 text-xl font-display font-bold text-white">{selectionOpen ? 'Ready to spin' : 'Selection is closed'}</h2><p className="mt-2 text-sm text-text-muted">{selectionOpen ? 'Your team can generate this assignment once. The result is saved permanently.' : 'Challenge selection will be opened by the event administrators.'}</p><ChallengeSpinWheel phase={phase} entries={isFeature ? undefined : situationWheelEntries} disabled={loading !== null || !selectionOpen} onSpinStart={() => isFeature && beginSpin('feature')} prepareSpin={isFeature ? undefined : prepareSituation} onSpinComplete={isFeature ? spinFeature : finishSituation} /></section>;
