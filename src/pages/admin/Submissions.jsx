...
export const Submissions = () => {
  ...
  return (
    <div className="p-8 space-y-6 max-w-5xl">
      <div>
        <h1 className="font-orbitron font-bold text-3xl text-[#39FF14] mb-1"><GlitchText text="SUBMISSIONS" /></h1>
        <p className="font-mono text-cp-muted text-sm">{subs.length} TOTAL · {scored} SCORED</p>
      </div>

      <div className="relative">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search project, team, track..."
          className="w-full bg-cp-dark border border-cp-border px-4 py-3 font-mono text-sm text-cp-text outline-none focus:border-[#39FF14] placeholder:text-cp-muted" />
      </div>

      {loading ? (
        <div className="font-mono text-cp-cyan animate-pulse">LOADING...</div>
      ) : filtered.length === 0 ? (
        <HoloPanel className="text-center py-12"><p className="font-mono text-cp-muted">NO SUBMISSIONS YET</p></HoloPanel>
      ) : (
        <div className="space-y-4">
          {filtered.map(s => (
            <HoloPanel key={s.id} className={s.judge_score !== null ? 'border-l-4 border-[#39FF14]' : ''}>
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3 flex-wrap">
                    <h3 className="font-orbitron text-white">{s.project_name}</h3>
                    {s.teams && <span className="font-mono text-[10px] bg-cp-magenta/10 text-cp-magenta px-2 py-0.5">{s.teams.name} · {s.teams.track}</span>}
                    {s.judge_score !== null && (
                      <span className="font-orbitron text-[#39FF14] text-sm flex items-center gap-1">
                        <Star size={12} />{s.judge_score}/100
                      </span>
                    )}
                  </div>
                  <p className="font-mono text-xs text-cp-muted">By: {s.profiles?.full_name}</p>
                  {s.description && <p className="font-mono text-xs text-cp-text line-clamp-2">{s.description}</p>}
                  ...
                  {s.judge_notes && <p className="font-mono text-xs text-cp-muted italic">Notes: "{s.judge_notes}"</p>}
                </div>
                  ...
                  {scoring === s.id ? (
                    <div className="space-y-2 min-w-[180px]">
                      ...
                      <input value={notes} onChange={e => setNotes(e.target.value)} placeholder="Judge notes"
                        className="w-full bg-cp-dark border border-cp-border px-3 py-2 font-mono text-xs text-cp-text outline-none focus:border-[#39FF14] placeholder:text-cp-muted" />
